...
def locate_biometric_portrait_exact(image_path: str, template_side: str, is_composite: bool, doc_type: str = "Aadhaar"):
    ...
    if yunet_success and yunet_bbox:
        # PAN cards allow faces on the left quadrant (x < 60.0, y < 85.0)
...
async def screen_document(file: UploadFile = File(...)):
    ...
    # ---------------------------------------------------------
    # MULTI-CREDENTIAL CLASSIFIER (PAN vs. AADHAAR)
    # ---------------------------------------------------------
    ...
    if is_pan and not is_aadhaar:
        ...
        # Fallback heuristic by regex signature
        if pan_matches:
            doc_type = "PAN Card"
            template_side = "FRONT_ONLY"
        else:
    ...
    # Safe Identifier Extraction & Masking by Document Class
    ...
    if doc_type == "PAN Card":
        ...
        # Aadhaar Verhoeff Permutation Check
        if raw_id:
