...
# ===========================================================
# 4. CAMERA PRNU SENSOR NOISE COLLISION ANALYZER
# ===========================================================

def extract_sensor_prnu_residual(img_grayscale: np.ndarray) -> np.ndarray:
    """
    Extracts sensor noise fingerprint using 2D Wavelet Denoising Filter.
    Falls back to Gaussian residual if PyWavelets is not yet installed.
    """
...
def detect_sensor_collision(image_path: str, face_bbox: dict) -> dict:
    """
    Compares physical camera noise of the face region vs background card canvas.
    If correlation approaches zero, the face was captured by a different hardware camera.
    """
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
    ...
    is_frankenstein_splice = bool(correlation < 0.05)
...
# ===========================================================
# 5. PRECISION BIOMETRIC PORTRAIT LOCATOR
# ===========================================================
...
# ===========================================================
# 6. DYNAMIC OCR AUTHORITY HEADER LOCATOR
# ===========================================================
...
async def screen_document(file: UploadFile = File(...)):
    ...
    # 1. Biometric Portrait Verification & PRNU Silicon Hardware Collision
    if face_found and face_coords:
        ...
        if prnu_result["sensor_tampering"]:
            ...
            risk_breakdown.append({
                ...
                "weight": 45,
                "impact": "negative",
                "description": f"Micro-noise cross-correlation ({prnu_result['prnu_sensor_correlation']}) detects multi-camera image splicing"
            })
            ai_explanations.append("PRNU sensor noise floor analysis confirms the biometric portrait originated from a completely different physical camera sensor than the card canvas.")
            ...
            risk_breakdown.append({
                ...
                "weight": -10,
                "impact": "positive",
                "description": f"Consistent silicon noise residual profile (Correlation: {prnu_result['prnu_sensor_correlation']})"
            })
