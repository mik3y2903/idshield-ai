...
# ===========================================================
# 4. SPATIAL RICH MODEL (SRM): GENERATIVE INPAINTING CHECK
# ===========================================================

def detect_generative_inpainting_srm(image_path: str) -> dict:
    try:
        img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
        ...
        is_inpainted = bool(anomaly_ratio > 0.85)
        ...
        return {
            ...
            "confidence": min(round(anomaly_ratio * 75, 1), 98.0),
            "bbox": bbox
        }
    except Exception as e:
...
def audit_typography_micro_alignment(image_path: str) -> dict:
    try:
        ...
        for i in range(len(glyph_boxes) - 1):
            ...
            if abs((y1 + h1) - (y2 + h2)) < 6 and (x2 - (x1 + w1)) < 25:
        ...
        is_tampered = bool(kerning_std > 5.5 or baseline_drift > 3.8)
...
# ===========================================================
# 6. ERROR LEVEL ANALYSIS (ELA) & DYNAMIC SPLICE LOCATOR
# ===========================================================
...
def perform_ela(image_path: str, quality: int = 90):
    try:
        ...
        # Dynamic localization of the highest-intensity splice contour
        ela_gray = cv2.cvtColor(ela_array, cv2.COLOR_RGB2GRAY)
        _, diff_thresh = cv2.threshold(ela_gray, 210, 255, cv2.THRESH_BINARY)
        ...
        if contours:
            ...
            if cv2.contourArea(largest_cnt) > 35:
...
# ===========================================================
# 7. MULTI-SCALE BIOMETRIC FACE DETECTOR (NO DUMMY FALLBACKS)
# ===========================================================
    ...
    try:
        ...
        if hasattr(cv2, 'CascadeClassifier') and hasattr(cv2, 'data'):
            ...
            if os.path.exists(cascade_path):
                ...
                # Multi-pass detection: standard pass, followed by high-contrast pass
                faces = face_cascade.detectMultiScale(gray, scaleFactor=1.08, minNeighbors=3, minSize=(30, 30))
                if len(faces) == 0:
                    faces = face_cascade.detectMultiScale(gray_enhanced, scaleFactor=1.05, minNeighbors=2, minSize=(25, 25))
...
# ===========================================================
# 8. OMNI-DIRECTIONAL QR / BARCODE LOCATOR
# ===========================================================
...
async def screen_document(file: UploadFile = File(...)):
    ...
    if ela_anomaly > 35:
        ...
        # Only attach region if the high-difference cluster was spatially localized
        if ela_bbox:
    ...
    # 4. Verified Biometric Face Alignment (No Dummy Regions)
    ...
    # 5. 2D-FFT Moiré Frequency Analysis (Screen Replay Detection)
    ...
    # 6. Quantization Tables & Editing Stamping (DQT)
    ...
    # 7. Spatial Rich Model (SRM) with Dynamic Coordinate Extraction
    srm_result = detect_generative_inpainting_srm(file_path)
    if srm_result["generative_fill_detected"]:
        ...
        ai_explanations.append("Spatial Rich Model (SRM) analysis detected generative inpainting or localized digital cloning across document fields.")
        
        # Only attach region if the anomalous variance grid was located
        if srm_result["bbox"]:
            evidence_regions.append({
                "id": "EV-SRM",
                ...
                "type": "pixel_splice",
                "explanation": "High-frequency noise suppression characteristic of AI generative fill."
            })
    ...
    if typo_result["typographic_tampering"]:
        ...
        ai_explanations.append(f"Sub-pixel typographic inspection detected non-standard kerning metrics ({typo_result['kerning_deviation']}px variance), confirming manual text insertion.")
