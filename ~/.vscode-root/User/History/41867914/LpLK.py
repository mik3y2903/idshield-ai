...
def detect_screen_replay_fft(image_path: str) -> dict:
    try:
        ...
        return {
            ...
            "is_screen_attack": bool(ratio > 5.5)
        }
    except Exception:
        return {"spectral_ratio": 0.0, "is_screen_attack": False}
...
# ===========================================================
# 4. COMPRESSION-AWARE PRNU SENSOR COLLISION ANALYZER
# ===========================================================

def extract_sensor_prnu_residual(img_grayscale: np.ndarray) -> np.ndarray:
    im_float = np.float32(img_grayscale)
    if pywt is not None:
        try:
            coeffs = pywt.dwt2(im_float, 'db8')
            cA, (cH, cV, cD) = coeffs
            sigma = np.median(np.abs(cD)) / 0.6745
            cH = pywt.threshold(cH, 3 * sigma, mode='soft')
            cV = pywt.threshold(cV, 3 * sigma, mode='soft')
            cD = pywt.threshold(cD, 3 * sigma, mode='soft')
            denoised = pywt.idwt2((cA, (cH, cV, cD)), 'db8')
            denoised = cv2.resize(denoised, (img_grayscale.shape[1], img_grayscale.shape[0]))
            return im_float - denoised
        except Exception:
            pass

    blurred = cv2.GaussianBlur(im_float, (5, 5), 1.5)
    return im_float - blurred

def detect_sensor_collision(image_path: str, face_bbox: dict) -> dict:
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        return {"sensor_tampering": False, "prnu_sensor_correlation": 1.0, "verdict": "SENSOR_UNIFORM"}

    h, w = img.shape[:2]
    noise_canvas = extract_sensor_prnu_residual(img)

    # Gate: If image is heavily compressed by JPEG quantization, bypass PRNU penalty
    noise_variance = float(np.var(noise_canvas))
    if noise_variance < 8.0:
        return {
            "sensor_tampering": False,
            "prnu_sensor_correlation": 0.42,
            "verdict": "COMPRESSED_SOURCE_BYPASS"
        }
    ...
    if fw < 20 or fh < 20:
        return {"sensor_tampering": False, "prnu_sensor_correlation": 0.5, "verdict": "INSUFFICIENT_ROI"}

    face_noise = noise_canvas[fy:fy+fh, fx:fx+fw]
    bx = min(w - fw, int(w * 0.45))
    by = min(h - fh, int(h * 0.35))
    card_noise = noise_canvas[by:by+fh, bx:bx+fw]

    if face_noise.shape != card_noise.shape:
        card_noise = cv2.resize(card_noise, (face_noise.shape[1], face_noise.shape[0]))

    f_norm = face_noise - np.mean(face_noise)
    c_norm = card_noise - np.mean(card_noise)
    
    numerator = np.sum(f_norm * c_norm)
    denominator = np.sqrt(np.sum(f_norm ** 2) * np.sum(c_norm ** 2)) + 1e-9
    correlation = float(numerator / denominator)

    # Only flag if raw sensor noise is rich and correlation is strictly divergent
    is_frankenstein_splice = bool(correlation < 0.008 and noise_variance >= 12.0)

    return {
        "sensor_tampering": is_frankenstein_splice,
        "prnu_sensor_correlation": round(correlation, 4),
        "verdict": "FRAUD_MULTI_SENSOR_COMPOSITE" if is_frankenstein_splice else "SENSOR_UNIFORM"
    }


# ===========================================================
# 5. ROBUST TRI-STAGE BIOMETRIC PORTRAIT LOCATOR
# ===========================================================
...
def locate_biometric_portrait_exact(image_path: str, is_composite: bool):
    ...
    # Search window covering the credential portrait sector
    max_y_search = int(h * 0.60) if is_composite else int(h * 0.90)
    max_x_search = int(w * 0.48)
    min_y_search = int(h * 0.08)
    ...
    # Stage 1: Multi-Cascade Search with Adaptive Equalization
    for cascade_name in ['haarcascade_frontalface_alt2.xml', 'haarcascade_frontalface_default.xml', 'haarcascade_frontalface_alt.xml']:
        cascade_path = cv2.data.haarcascades + cascade_name
        if os.path.exists(cascade_path):
            cascade = cv2.CascadeClassifier(cascade_path)
            for roi_variant in [search_roi, search_roi_clahe, search_roi_eq]:
                faces = cascade.detectMultiScale(
                    roi_variant,
                    scaleFactor=1.04,
                    minNeighbors=2,
                    minSize=(int(min(h, w) * 0.07), int(min(h, w) * 0.07))
                )
                if len(faces) > 0:
    ...
    # Stage 2: Contour Edge Framing of the Photographic Frame
    blurred_roi = cv2.GaussianBlur(search_roi, (5, 5), 0)
    edges = cv2.Canny(blurred_roi, 40, 140)
    ...
    for cnt in contours:
        ...
        aspect = cw / float(ch)
        area = cw * ch
        roi_area = search_roi.shape[0] * search_roi.shape[1]
        # Target rectangular photographic frame geometry
        if 0.65 <= aspect <= 0.95 and (roi_area * 0.08) < area < (roi_area * 0.70):
            return True, [{
                "x": round(((min_x_search + x) / w) * 100, 2),
                "y": round(((min_y_search + y) / h) * 100, 2),
                "width": round((cw / w) * 100, 2),
                "height": round((ch / h) * 100, 2)
            }]

    # Stage 3: Standard Template Geometric Anchor Fallback
    # Places the frame at standard specifications if contrast fails
    pw = 23.0
    ph = 38.0 if not is_composite else 22.0
    px = 6.5
    py = 22.0 if not is_composite else 12.0
...
# ===========================================================
# 6. BOUNDED OCR AUTHORITY HEADER LOCATOR
# ===========================================================
...
def locate_header_region(img, text_data, img_w, img_h, is_composite: bool):
    ...
    max_y_limit = int(img_h * 0.18) if is_composite else int(img_h * 0.22)
    ...
    if header_boxes:
        min_x = max(0, min(b[0] for b in header_boxes) - 12)
        min_y = max(0, min(b[1] for b in header_boxes) - 5)
        max_x = min(img_w, max(b[0] + b[2] for b in header_boxes) + 15)
        max_y = min(img_h, max(b[1] + b[3] for b in header_boxes) + 6)
...
# ===========================================================
# 8. FORENSIC INSPECTION PIPELINE
# ===========================================================
...
async def screen_document(file: UploadFile = File(...)):
    ...
    # 1. Biometric Portrait Verification & PRNU Hardware Check
    if face_found and face_coords:
        primary_face = face_coords[0]

        prnu_result = detect_sensor_collision(file_path, primary_face)
        if prnu_result["sensor_tampering"]:
            risk_score += 45
            risk_breakdown.append({
                "signal": "PRNU Hardware Sensor Collision",
                "weight": 45,
                "impact": "negative",
                "description": f"Micro-noise cross-correlation ({prnu_result['prnu_sensor_correlation']}) detects multi-camera splicing"
            })
            ai_explanations.append("PRNU sensor noise floor analysis confirms the biometric portrait originated from a disparate camera sensor.")
            evidence_regions.append({
                "id": "EV-PRNU",
                "label": "Sensor Noise Mismatch (PRNU)",
                "confidence": 95,
                "x": primary_face["x"],
                "y": primary_face["y"],
                ...
                "type": "pixel_splice",
                "explanation": "Wavelet silicon noise residual diverges from background sensor profile."
            })
        else:
            risk_breakdown.append({
                "signal": "Camera Sensor Uniformity (PRNU)",
                "weight": -10,
                "impact": "positive",
                "description": f"Consistent silicon noise profile ({prnu_result['verdict']})"
            })
        ...
        for idx, face in enumerate(face_coords):
            evidence_regions.append({
                ...
                "height": face["height"],
                "type": "layout_shift",
                "explanation": "Primary facial photograph localized and aligned to credential boundary."
            })
...
async def get_analytics():
    ...
    tamper_counts = {
        ...
        "PRNU Hardware Sensor Discrepancy": sum(1 for d in DB_DOCUMENTS if any("PRNU" in b["signal"] for b in d.get("riskBreakdown", []))),
