...
def detect_screen_replay_fft(image_path: str) -> dict:
    try:
        ...
        return {
            ...
            "is_screen_attack": bool(ratio > 4.2)
        }
...
# ===========================================================
# 3. METADATA & DQT QUANTIZATION FINGERPRINTING
# ===========================================================
...
# ===========================================================
# 4. SPATIAL RICH MODEL (SRM) - WITH QR ZONE SUPPRESSION
# ===========================================================

def detect_generative_inpainting_srm(image_path: str, qr_bbox=None) -> dict:
    try:
        img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            return {"generative_fill_detected": False, "noise_variance": 0.0, "confidence": 0, "bbox": None}
        ...
        residual = cv2.filter2D(img, -1, srm_kernel)
        h, w = residual.shape
        ...
        for r in range(4):
            for c in range(4):
                ...
                # Suppress the grid block if it overlaps with the detected QR code
                if qr_bbox and abs(bx - qr_bbox['x']) < 20 and abs(by - qr_bbox['y']) < 20:
                    continue

                block = residual[r*grid_h:(r+1)*grid_h, c*grid_w:(c+1)*grid_w]
        ...
        var_std = float(np.std(local_variances))
        var_mean = float(np.mean(local_variances)) + 1e-5
        anomaly_ratio = round(var_std / var_mean, 3)
        is_inpainted = bool(anomaly_ratio > 1.25)

        bbox = None
        if is_inpainted:
            max_idx = int(np.argmax(local_variances))
            r_max, c_max = coords[max_idx]
            bbox = {
                "x": round((c_max * grid_w / w) * 100, 2),
                "y": round((r_max * grid_h / h) * 100, 2),
                "width": round((grid_w / w) * 100, 2),
                "height": round((grid_h / h) * 100, 2)
            }

        return {
            "generative_fill_detected": is_inpainted,
            "noise_discrepancy_ratio": anomaly_ratio,
            "confidence": min(round(anomaly_ratio * 60, 1), 95.0),
            "bbox": bbox
        }
    except Exception as e:
        print(f"[WARN] SRM Error: {e}")
        return {"generative_fill_detected": False, "noise_variance": 0.0, "confidence": 0, "bbox": None}


# ===========================================================
# 5. SUB-PIXEL TYPOGRAPHIC BASELINE & KERNING AUDIT
# ===========================================================

def audit_typography_micro_alignment(image_path: str) -> dict:
    try:
        img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            return {"typographic_tampering": False, "kerning_deviation": 0.0, "baseline_drift_px": 0.0}
        ...
        for cnt in contours:
            ...
            if 10 < h < 45 and 6 < w < 35:
                glyph_boxes.append((x, y, w, h))

        if len(glyph_boxes) < 8:
            return {"typographic_tampering": False, "kerning_deviation": 0.0, "baseline_drift_px": 0.0}
        ...
        for i in range(len(glyph_boxes) - 1):
            x1, y1, w1, h1 = glyph_boxes[i]
            x2, y2, w2, h2 = glyph_boxes[i+1]
            if abs((y1 + h1) - (y2 + h2)) < 5 and (x2 - (x1 + w1)) < 20:
                kernings.append(x2 - (x1 + w1))
                baseline_y.append(y1 + h1)

        if not kernings:
            return {"typographic_tampering": False, "kerning_deviation": 0.0, "baseline_drift_px": 0.0}

        kerning_std = float(np.std(kernings))
        baseline_drift = float(np.std(baseline_y)) if baseline_y else 0.0
        is_tampered = bool(kerning_std > 6.5 or baseline_drift > 4.5)

        return {
            "typographic_tampering": is_tampered,
            "kerning_deviation": round(kerning_std, 2),
            "baseline_drift_px": round(baseline_drift, 2)
        }
    except Exception as e:
        print(f"[WARN] Typography Error: {e}")
        return {"typographic_tampering": False, "kerning_deviation": 0.0, "baseline_drift_px": 0.0}
...
def perform_ela(image_path: str, quality: int = 90):
    try:
        ...
        _, diff_thresh = cv2.threshold(ela_gray, 220, 255, cv2.THRESH_BINARY)
        ...
        if contours:
            ...
            if cv2.contourArea(largest_cnt) > 80:
        ...
        return min(round(anomaly_score * 1.5, 2), 100.0), bbox
    except Exception as e:
...
# ===========================================================
# 7. MULTI-SCALE BIOMETRIC FACE DETECTOR (SCALE ADAPTIVE)
# ===========================================================
    ...
    try:
        ...
        if hasattr(cv2, 'CascadeClassifier') and hasattr(cv2, 'data'):
            ...
            if os.path.exists(cascade_path):
                ...
                # Multi-pass: accommodates both large card photos and small composite-sheet photos
                faces = face_cascade.detectMultiScale(gray, scaleFactor=1.05, minNeighbors=3, minSize=(20, 20))
                if len(faces) == 0:
                    faces = face_cascade.detectMultiScale(gray_enhanced, scaleFactor=1.04, minNeighbors=2, minSize=(16, 16))
...
# ===========================================================
# 8. PRECISE QR LOCATOR (NO BLIND CONTOUR GUESSING)
# ===========================================================
        ...
        for p in passes:
            ...
            if points is not None and len(points) > 0:
                ...
                if (x_max - x_min) > 20 and (y_max - y_min) > 20:
                    ...
                    return True, val if val else "VALID_2D_MATRIX", bbox

    return False, None, None


# ===========================================================
# 9. OCR & REDACTED CLASSIFIER
# ===========================================================
...
# ===========================================================
# 10. REAL-TIME FORENSIC SCREENING PIPELINE
# ===========================================================
...
async def screen_document(file: UploadFile = File(...)):
    ...
    ela_anomaly, ela_bbox = perform_ela(file_path)
    ...
    # 2. Tampering & Dynamic ELA Bounding
    if ela_anomaly > 45:
        tamper_weight = min(int(ela_anomaly * 0.4), 25)
        ...
        if ela_bbox:
            evidence_regions.append({
                ...
                "x": ela_bbox["x"],
                "y": ela_bbox["y"],
                "width": ela_bbox["width"],
                "height": ela_bbox["height"],
                "type": "pixel_splice",
                "explanation": "Discrete cosine transform (DCT) anomalies localized in this sector."
            })
    else:
    ...
    # 3. Dynamic Omni-Directional QR / Barcode Check
    ...
    # 4. Verified Biometric Face Alignment
    if face_found and face_coords:
        risk_breakdown.append({
            "signal": "Biometric Face Match",
            "weight": -10,
            "impact": "positive",
            "description": "Facial portrait geometry identified and located"
        })
        for idx, face in enumerate(face_coords):
            evidence_regions.append({
                ...
                "height": face["height"],
                "type": "layout_shift",
                "explanation": "Facial portrait verified and anchored to detected facial landmarks."
            })
    else:
        risk_score += 20
        risk_breakdown.append({
        ...
        ai_explanations.append("Facial portrait verification failed. Target card lacks a clear biometric photo.")

    # 5. 2D-FFT Moiré Frequency Analysis
    ...
    # 6. Quantization Tables & Editing Metadata (DQT)
    ...
    # 7. Spatial Rich Model (SRM) - with QR Box Suppression
    srm_result = detect_generative_inpainting_srm(file_path, qr_bbox=qr_bbox)
    if srm_result["generative_fill_detected"]:
        risk_score += 30
        risk_breakdown.append({
            "signal": "Generative AI Inpainting / Neural Fill",
            "weight": 30,
            "impact": "negative",
            "description": f"SRM noise divergence ({srm_result['noise_discrepancy_ratio']}) exposes neural diffusion manipulation"
        })
        ai_explanations.append("Spatial Rich Model (SRM) analysis detected localized noise suppression characteristic of AI generative inpainting.")
        if srm_result["bbox"]:
            evidence_regions.append({
                "id": "EV-SRM",
                "label": "AI Generative Fill / Inpainting",
                "confidence": int(srm_result["confidence"]),
                "x": srm_result["bbox"]["x"],
                "y": srm_result["bbox"]["y"],
                "width": srm_result["bbox"]["width"],
                "height": srm_result["bbox"]["height"],
                "type": "pixel_splice",
                "explanation": "High-frequency noise suppression localized in this canvas block."
            })

    # 8. Micro-Typographic Kerning & Baseline Alignment
    typo_result = audit_typography_micro_alignment(file_path)
    if typo_result["typographic_tampering"]:
        risk_score += 20
        risk_breakdown.append({
            "signal": "Micro-Typographic Kerning Drift",
            "weight": 20,
            "impact": "negative",
            "description": f"Baseline drift of {typo_result['baseline_drift_px']}px indicates desktop font substitution"
        })
        ai_explanations.append(f"Sub-pixel typographic inspection detected non-standard kerning metrics ({typo_result['kerning_deviation']}px variance).")

    # 9. Dihedral D5 Verhoeff Checksum Check
    if doc_type == "Aadhaar":
        numbers = re.findall(r'\b\d{4}\s?\d{4}\s?\d{4}\b', raw_text)
        if numbers:
            raw_id = numbers[0].replace(" ", "")
            is_valid_verhoeff = validate_verhoeff(raw_id)
    ...
    result = {
        ...
        "tamperingDetected": tampering_detected or srm_result["generative_fill_detected"] or typo_result["typographic_tampering"],
        "qrStatus": qr_status,
        "status": status,
        "timestamp": datetime.now().strftime("%d %b %Y, %H:%M:%S IST"),
        "processedByNode": "KALI-INFERENCE-LOCAL-01",
        "layoutIntegrity": 85 if not (tampering_detected or typo_result["typographic_tampering"]) else 58,
